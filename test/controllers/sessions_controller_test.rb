...
class SessionsControllerTest < ActionDispatch::IntegrationTest
  ...
  def test_should_login_user_with_valid_credentials
    post api_v1_sessions_path, params: { login: { email: @user.email, password: @user.password } }, as: :json
    assert_response :success
    assert_equal @user.authentication_token, response.parsed_body["user"]["authentication_token"]
  end

  def test_shouldnt_login_user_with_invalid_credentials
    post api_v1_sessions_path, params: { login: { email: @user.email, password: "invalid password" } }, as: :json
  ...
  def test_should_respond_with_not_found_error_if_user_is_not_present
    ...
    post api_v1_sessions_path, params: { login: { email: non_existent_email, password: "welcome" } }, as: :json
